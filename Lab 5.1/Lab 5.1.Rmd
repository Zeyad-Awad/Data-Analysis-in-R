...
A we can see, \~0.066 of those who never wears a helmet texted every day in the past 30 days. Also, of all the observations, \~0.034 never wears a helmet and texted everyday for 30 days.
...
Based on this survey, the margin of error, ME, is ME = 0.06.

```{r ex3}
no_helmet%>%filter(complete.cases(text_30))%>%
  specify(response = text_30, success = "yes")%>%
  generate(reps = 1000, type = "bootstrap")%>%
  calculate(stat = "prop")%>%
  get_ci(level = 0.95)
(0.07750269 - 0.06519683)
```

### Exercise 4

From this data, we are 95% confident that the population proportion, of highschoolers who scored a grade of 10, who sleep less than 5 hours a day is between 0.058 and 0.077. Which means that our margin of error, ME, is ME = 0.010. Our sampling distribution provides an estimate, p_hat, of 0.061.
...
grade_10%>%filter(complete.cases(sleep_less_5))%>%
...
### Exercise 6
It is roughly symmetrical and normally distributed around centre = 0.1 and the distribution is roughly between 0.06 and 0.15.

### Exercise 7
At p = 0.5, the distribution is normally distributed around centre = 0.5 and roughly symmetrical, and the spread is larger than the previous. The distribution is approximately between 0.4 and 0.6.

### Exercise 8
As n increases the spread decreases, i.e., the values are more centred on the population proportion. Also, the shape is more symmetrical and normally distributed. Whereas, smaller values of n makes the distribution look more discrete and adds some skewness to the shape.
...
<br></br>
For a 95% confidence interval, we expect that the difference in proportions of those who sleep more than 10 hours and those who do not, is between 0.0570 and 0.154. As depicted by the histogram of our differences of sample proportions, we would not observe a difference of 0 in the confidence interval, hence, there is evidence that those who train every day of the week are more likely to strength train every day of the week.
```{r ex9}
yrbss<- yrbss %>%
...
boots <- yrbss%>%
  filter(complete.cases(strength_7d, sleep_10))%>%
